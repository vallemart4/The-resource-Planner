...
let state = {
  ...
  editingMemberId: null, emName:'', emCountry:'Sweden', emSkill:'', emLevel:'Junior', emTeamlead:'', emManager:'',
  ...
  dashAllocRange: 8, showAllWeeks: false, weekOffset: 0,
  msg: null,
};
...
function startEditMember(id){
  ...
  state.editingMemberId=id; state.emName=m.name; state.emCountry=m.country; state.emSkill=m.skillset; state.emLevel=m.level; state.emTeamlead=m.teamlead||''; state.emManager=m.manager||''; render();
}
function saveMemberEditFromInputs(id){
  ...
  state.teamMembers=state.teamMembers.map(m => m.id===id ? {...m,name:nv,country:state.emCountry,skillset:sv,level:state.emLevel,teamlead:tv,manager:mv} : m);
  state.assignments=state.assignments.map(a => { if(a.name.trim().toLowerCase()===oldName) return {...a,name:nv,country:state.emCountry||a.country,skillset:sv||a.skillset,level:state.emLevel||a.level}; return a; });
