...
function getEffectiveAlloc(a, w){
  ...
  const svc = state.baseServices.find(s => s.name === a.workName);
  if(svc && svc.noReduce) return raw; // rotational/on-call services: count in full, never reduced
...
function toggleSvcNoReduce(name){
  state.baseServices = state.baseServices.map(s => s.name===name ? {...s, noReduce: !s.noReduce} : s);
  flashMsg(state.baseServices.find(s=>s.name===name).noReduce ? 'Marked as rotational/on-call — counts in full now.' : 'Back to normal reduction logic.', true);
  render();
}
