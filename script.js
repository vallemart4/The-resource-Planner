      ...
      ${visAssignments.length===0?`<div class="empty"><span class="empty-icon">👤</span>No resources assigned yet. Add one above!</div>`:`<div class="tbl-wrap"><table>
        ...
        <tbody>${visAssignments.map(a=>{
          ...
          const periodRows=a.periods.map((p,pi)=>{
            const pWeeks=WEEKS.map(w=>{ const inRange=w>=p.startWeek&&w<=p.endWeek; return `<td class="wk${w===CURRENT_WEEK?' wk-now':''}" style="font-size:10px;${inRange?'background:rgba(29,158,117,0.08);color:#0f6e56':''}">${inRange?p.allocationPercent+'%':''}</td>`; }).join('');
...
function renderTeamDetail(){
  ...
  function assignmentRows(person){
    ...
    return person.assignments.map(a=>{
      ...
      return `<tr style="background:#fafafa"><td style="padding:5px 12px 5px 28px;font-size:12px;color:#374151;white-space:nowrap">${label}</td><td style="padding:5px 12px;font-size:11px;color:#9ca3af">${a.type}</td><td colspan="2" style="padding:5px 12px;white-space:nowrap">${statusDot}</td><td style="padding:5px 12px"></td>${weekCells}${ce?`<td style="padding:5px 8px"></td>`:''}</tr>`;
    }).join('');
  }
...
function renderOverview(){
  ...
  if(!people.length){
    ...
    rows=`<div class="tbl-wrap"><table><thead><tr><th>Person</th><th>Team</th><th>Country</th><th>Skill</th><th>Level</th><th>Status</th>${WEEKS.map(w=>wkHdr(w)).join('')}</tr></thead><tbody>${people.map(p=>{
      ...
      return `<tr class="${ce?'person-row-click':''}" ${clickAttr}><td style="background:${cBg(p.country)}"><strong>${dn}</strong>${ce?`<span style="font-size:10px;color:#9ca3af;margin-left:6px">→</span>`:''}</td><td><span style="cursor:pointer;color:#1D9E75;text-decoration:underline" onclick="event.stopPropagation();openTeam('${p.team}')">${p.team}</span></td><td>${p.country}</td><td>${p.skillset}</td><td>${p.level}</td><td>${statusBadge}</td>${WEEKS.map(w=>wkCell(w,getTotalAlloc(p.name,w))).join('')}</tr>`;
    }).join('')}</tbody></table></div>`;
  }
  return `
